<template>
    <div class="user-list">
        ...
        <a-form-model ref="ruleForm" :model="form" :label-col="labelCol" :wrapper-col="wrapperCol" :rules="rules">
            ...
            <a-form-model-item label="摘要" prop="desc">
                <a-textarea v-model="form.desc" placeholder="请输入摘要" :auto-size="{ minRows: 4, maxRows: 8 }"/>
            </a-form-model-item>
    ...
    onSubmit() {
        (this.$refs['ruleForm'] as FormModel).validate((valid: boolean) => {
            if (valid) {
                ...
                http.post('/api/news/add',params).then(res=>{
